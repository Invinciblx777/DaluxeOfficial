                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {item.expandable && (
                      <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded cursor-pointer" style={{ background: 'rgba(212,175,55,0.1)', color: '#D4AF37' }}>
                        {showSupaConfig ? 'Hide Config' : 'Setup Credentials'}
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                        ...
                        onClick={async () => {
                          if (typeof window !== 'undefined') {
                            try {
                              ...
                              const data = await res.json();
                              if (data.success) {
                                window.localStorage.removeItem('NEXT_PUBLIC_SUPABASE_URL');
                                window.localStorage.removeItem('NEXT_PUBLIC_SUPABASE_ANON_KEY');
                                setSupabaseUrl(process.env.NEXT_PUBLIC_SUPABASE_URL || '');
                                setSupabaseAnonKey(process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '');
                                toast.success("Cleared custom overrides and env files! Reloading...");
                                window.dispatchEvent(new Event('storage'));
                                setTimeout(() => {
                                  window.location.reload();
                                }, 1000);
                              } else {
                                toast.error(`Failed to clear config on server: ${data.error}`);
                              }
                            } catch (e: any) {
                              toast.error(`Error clearing config: ${e.message}`);
                            }
                          }
                        }} 
                        className="px-4 py-2 text-xs font-bold rounded-lg text-zinc-400 hover:text-white transition-all"
